...
export function imapHandleRecieved(belongs_to, exists, updated_mailbox) {
    const filtered_connections = connections.filter(connection => connection.user.email == belongs_to && connection.active && connection.idle);

    for (let connection of filtered_connections)
        if (connection.mailbox == updated_mailbox)
