...
export async function setDB(name) {
    ...
    await db.execute('CREATE TABLE IF NOT EXISTS mailboxes (mailbox_id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, belongs_to TEXT NOT NULL, name TEXT NOT NULL, uid INT UNSIGNED DEFAULT 0, uid_next INT UNSIGNED DEFAULT 1)');
    await db.execute('CREATE TABLE IF NOT EXISTS emails (email_id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, belongs_to TEXT NOT NULL, mail_to TEXT NOT NULL, mail_from TEXT NOT NULL, name_from TEXT, reply_to TEXT NOT NULL, bcc TEXT NOT NULL, cc TEXT NOT NULL, mail_id TEXT NOT NULL, message_id TEXT, subject TEXT, content TEXT, attachments JSON, html_format TEXT, time BIGINT NOT NULL, email_references JSON, mail_box INT UNSIGNED NOT NULL, seen INT DEFAULT 0, deleted INT DEFAULT 0, uid INT UNSIGNED)');
}
